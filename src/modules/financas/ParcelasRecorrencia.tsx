...
export const ParcelasRecorrencia: React.FC<ParcelasRecorrenciaProps> = ({ transactions, onUpdateTransactions }) => {
  ...
  const [hideCompleted, setHideCompleted] = useState<boolean>(() => {
    return localStorage.getItem('financas_hide_completed_contracts') === 'true';
  });

  const handleToggleHideCompleted = () => {
    setHideCompleted(prev => {
      const next = !prev;
      localStorage.setItem('financas_hide_completed_contracts', String(next));
      return next;
    });
  };
  ...
  const activeGroups = groupsList.filter(g => g.pendingAmount > 0);
  const completedGroups = groupsList.filter(g => g.pendingAmount === 0);

  // Group active groups by category
  ...
  const renderExpenseCard = (group: GroupedExpense) => {
    ...
    const isCompleted = group.pendingAmount === 0;
    ...
    return (
      <div 
        ...
        className={`bg-white dark:bg-[#121214] border border-zinc-200 dark:border-zinc-800/50 rounded-2xl shadow-sm overflow-hidden transition-all duration-300 hover:border-zinc-300 dark:hover:border-zinc-700/50 ${
          isCompleted ? 'opacity-85 dark:opacity-80' : ''
        }`}
      >
        ...
        >
          <div className="flex items-center gap-3">
            <div className={`p-2.5 rounded-xl ${
              isCompleted 
                ? 'bg-zinc-100 dark:bg-zinc-850 text-zinc-400 dark:text-zinc-550' 
                : group.type === 'parcelamento' 
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className={`text-sm font-bold ${isCompleted ? 'text-zinc-500 line-through' : 'text-zinc-800 dark:text-white'}`}>
                  {group.baseName}
                </h3>
                {isCompleted && (
                  <span className="bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 dark:text-emerald-455 border border-emerald-100 dark:border-emerald-900/30 text-[8px] font-black uppercase px-2 py-0.5 rounded">
                    Quitado
                  </span>
                )}
              </div>
              <div className="flex flex-wrap items-center gap-2 mt-1">
                <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded ${
                  isCompleted 
                    ? 'bg-zinc-105 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400' 
                    : group.type === 'parcelamento' 
          ...
          <div className="flex flex-wrap items-center gap-6 w-full md:w-auto">
            ...
            <div className="flex flex-col gap-1 w-32">
              ...
              <div className="w-full h-1.5 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
                <div className={`h-full transition-all duration-500 ${isCompleted ? 'bg-emerald-600 dark:bg-emerald-500' : 'bg-emerald-500'}`} style={{ width: `${progressPercent}%` }} />
              </div>
            </div>
            ...
            <div className="flex items-center gap-4 text-right">
              <div>
                ...
                <p className={`text-xs font-extrabold ${isCompleted ? 'text-zinc-500' : 'text-emerald-500'}`}>{formatCurrency(group.items[0]?.amount || 0)}</p>
              </div>
              <div>
                <p className="text-[8px] font-black uppercase text-zinc-400 dark:text-zinc-500">Restante</p>
                <p className={`text-xs font-bold ${isCompleted ? 'text-zinc-400' : 'text-zinc-800 dark:text-white'}`}>{formatCurrency(group.pendingAmount)}</p>
              </div>
              <div>
  ...
  return (
    <div className="space-y-6 max-w-[1200px] mx-auto pb-12 animate-in fade-in duration-500">
      ...
      {/* Barra de Título & Toggle de Concluídos */}
      <div className="flex justify-between items-center bg-white dark:bg-[#121214] border border-zinc-200 dark:border-zinc-800/50 p-4 rounded-2xl shadow-sm">
        <div>
          <h2 className="text-sm font-black text-zinc-800 dark:text-white uppercase tracking-tight">Contratos de Parcelamentos & Recorrências</h2>
          <p className="text-[10px] text-zinc-450 dark:text-zinc-500 font-medium mt-0.5">Gerencie os fluxos de despesas divididas ou recorrentes.</p>
        </div>
        
        <button
          type="button"
          onClick={handleToggleHideCompleted}
          className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center gap-1.5 shadow-sm border ${
            hideCompleted
              ? 'bg-rose-50 border-rose-200 text-rose-600 dark:bg-rose-950/20 dark:border-rose-900/30 dark:text-rose-455'
              : 'bg-white border-zinc-200 text-zinc-700 dark:bg-[#121214] dark:border-zinc-800 dark:text-zinc-350 hover:bg-zinc-50 dark:hover:bg-zinc-800/30'
          }`}
        >
      ...
      {activeGroups.length === 0 && (hideCompleted || completedGroups.length === 0) ? (
        <div className="bg-white dark:bg-[#121214] border border-zinc-200 dark:border-zinc-800/50 rounded-2xl p-12 text-center shadow-sm">
          <p className="text-sm font-semibold text-zinc-500 dark:text-zinc-450">Nenhum gasto parcelado ou recorrente ativo identificado.</p>
          <p className="text-xs text-zinc-400 dark:text-zinc-550 mt-1">Lançamentos criados com parcelas ou recorrência no cartão de crédito/Pix aparecerão aqui.</p>
        </div>
      ) : (
        <div className="space-y-8 animate-in fade-in duration-350">
          {/* Listagem de Ativos por Categoria */}
                ...
                return (
                  <div key={categoryName} className="space-y-3">
                    ...
                    <div className="space-y-4">
                      {categoryGroups.map(group => renderExpenseCard(group))}
                    </div>
                  </div>
                );
              })}
          ...
          {/* Listagem de Concluídos (se não estiverem ocultados) */}
          {!hideCompleted && completedGroups.length > 0 && (
            <div className="space-y-4 pt-8 border-t border-zinc-200 dark:border-zinc-800/80">
              <div className="flex items-center justify-between px-1">
                <div>
                  <h3 className="text-xs font-black uppercase tracking-wider text-zinc-550 dark:text-zinc-450">
                    Contratos Concluídos (Quitados)
                  </h3>
                  <p className="text-[9px] text-zinc-455 dark:text-zinc-500 font-medium">Contratos onde todas as parcelas foram pagas.</p>
                </div>
                <span className="text-[9px] bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 dark:text-emerald-450 border border-emerald-100 dark:border-emerald-900/30 px-2.5 py-0.5 rounded-full font-bold">
                  {completedGroups.length} quitado{completedGroups.length === 1 ? '' : 's'}
                </span>
              </div>
              <div className="space-y-4">
                {completedGroups.map(group => renderExpenseCard(group))}
              </div>
            </div>
          )}
        </div>
