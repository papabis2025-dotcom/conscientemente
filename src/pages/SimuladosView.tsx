  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl text-slate-800 dark:text-white">Simulados Completos</h2>
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-xl animate-in slide-in-from-top-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
              ...
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Ex: Simulado Ciclo 1" className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 text-slate-800 dark:text-white" />
            </div>
            <div>
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 block">Data</label>
              <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500 text-slate-800 dark:text-white" />
            </div>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl border border-slate-200 dark:border-slate-700 mb-8">
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              ...
              <input type="number" value={currentDone} onChange={(e) => setCurrentDone(e.target.value)} placeholder="Total" className="w-24 px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-sm dark:text-white" />
              <input type="number" value={currentCorrect} onChange={(e) => setCurrentCorrect(e.target.value)} placeholder="Acertos" className="w-24 px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl outline-none text-sm dark:text-white" />
              <button onClick={addResultRow} className="bg-blue-600 text-white px-6 py-2 rounded-xl font-bold text-xs uppercase hover:bg-blue-700">Add</button>
            </div>
            ...
            <div className="space-y-2">
              {results.map((res, i) => {
                ...
                return (
                  <div key={i} className="flex items-center justify-between p-3 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
                    <div className="flex items-center gap-3">
                      <div className={`w-2 h-8 rounded-full ${sub?.color || 'bg-slate-500'}`} />
                      <div>
                        <p className="text-xs font-black dark:text-white leading-none">{sub?.name}</p>
                        <p className="text-[10px] text-slate-400 mt-1">{res.correct} acertos de {res.done} questões ({acc}%)</p>
                      </div>
                    </div>
          ...
          <div className="flex justify-end gap-3">
            <button onClick={() => setIsAdding(false)} className="px-6 py-3 text-slate-400 font-bold uppercase text-xs">Cancelar</button>
          ...
          return (
            <div key={sim.id} className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 p-6 shadow-sm hover:shadow-xl transition-all group">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h4 className="text-lg font-black text-slate-800 dark:text-white leading-tight">{sim.name}</h4>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">{new Date(sim.date).toLocaleDateString('pt-BR')}</p>
                </div>
                <button onClick={() => confirm('Excluir simulado?') && onDeleteSimulado(sim.id)} className="text-slate-200 group-hover:text-rose-500 transition-colors">🗑️</button>
              </div>
