...
export const ParcelasRecorrencia: React.FC<ParcelasRecorrenciaProps> = ({ transactions, onUpdateTransactions }) => {
  ...
  transactions.forEach(t => {
    if (t.type === 'saida' && !t.name.match(/(.*) \((\d+)\/(\d+)\)$/)) {
  ...
  transactions.forEach(t => {
    ...
    const installmentMatch = t.name.match(/(.*) \((\d+)\/(\d+)\)$/);
    if (installmentMatch) {
      ...
      {groupsList.length === 0 ? (
        ...
        <div className="space-y-4">
          {groupsList.map(group => {
            ...
            const paidCount = group.items.filter(item => !item.pending).length;
                  ...
                  <div className="flex flex-wrap items-center gap-6 w-full md:w-auto">
                    ...
                    <div className="flex items-center gap-4 text-right">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-bold text-zinc-500 dark:text-zinc-400">{formatCurrency(group.totalAmount)}</p>
                      </div>
                    </div>
                      ...
                      <table className="w-full text-left text-xs">
                        ...
                        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/50">
                          {group.items.map((item, idx) => {
                            // Extract installment index if exists, otherwise fallback to item index
