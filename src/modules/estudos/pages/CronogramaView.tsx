...
const CronogramaView: React.FC<CronogramaViewProps> = ({
  ...
  const handleToggleCronogramaEnabled = async (enabled: boolean) => {
    ...
    try {
      ...
      if (!enabled) {
        const autoGeneratedTasks = (scheduledStudies || []).filter(s => 
          s.generatedByCronograma && s.status !== 'realizado' &&
      ...
      console.error('Error toggling cronograma state:', e);
      setIsCronogramaEnabled(previousState);
      alert('Ocorreu um erro ao alterar o estado do cronograma. As alterações foram revertidas.');
    }
  };
  ...
  // Reset/Clear schedule generated tasks only (preserving completed & manual tasks)
  const handleResetAll = async () => {
    if (!selectedConcursoId || selectedConcursoId === 'all') return;
    if (confirm('Tem certeza de que deseja zerar as tarefas não concluídas geradas pelo cronograma? Suas tarefas marcadas como realizadas e tarefas manuais serão mantidas.')) {
      if (onResetConcursoSchedule) {
        await onResetConcursoSchedule(selectedConcursoId);
        window.dispatchEvent(new Event('local-settings-changed'));
        alert('Tarefas não concluídas geradas pelo cronograma foram removidas! Suas tarefas concluídas e estudos manuais permanecem salvos.');
      }
    }
  };
  ...
  const handleClearSchedule = async () => {
    if (confirm('Tem certeza de que deseja apagar as tarefas pendentes do cronograma de estudos gerado automaticamente? Suas tarefas manuais e tarefas concluídas não serão afetadas.')) {
