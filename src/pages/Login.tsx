...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      setError(err.message || 'Erro ao conectar com Google OAuth.');
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-150 font-sans selection:bg-blue-500/20 overflow-x-hidden relative transition-colors duration-300">
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="bg-zinc-900 text-white dark:bg-white dark:text-zinc-900 px-4 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-wider hover:bg-blue-600 dark:hover:bg-blue-55 transition-all shadow-lg active:scale-95"
            >
              Criar Conta
            </button>
          </div>
      ...
      <section className="max-w-7xl mx-auto px-6 pt-28 pb-16 md:pt-36 md:pb-24 flex flex-col lg:flex-row items-center gap-12 relative">
        ...
        <div className="flex-1 space-y-6 text-left relative z-10">
          ...
          <div className="flex items-center gap-6 pt-6 border-t border-zinc-200/50 dark:border-zinc-855/50 max-w-md">
            <div>
            ...
            {activePreviewTab === 'planner' && (
              <div className="space-y-6 animate-in fade-in duration-300">
                <div className="flex items-center justify-between pb-4 border-b border-zinc-100 dark:border-zinc-800">
                  <div>
                    <h3 className="text-sm font-black uppercase tracking-wider text-zinc-855 dark:text-white">Cronograma Inteligente</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="bg-zinc-50 dark:bg-zinc-900/35 border border-zinc-200/40 dark:border-zinc-800/40 rounded-2xl p-4 space-y-3 hover:scale-[1.02] transition-transform duration-300">
                    ...
                    <div className="p-3 bg-indigo-50/50 dark:bg-indigo-955/10 border border-indigo-200/25 dark:border-indigo-900/20 rounded-xl space-y-1.5">
            ...
            {activePreviewTab === 'stats' && (
              <div className="space-y-6 animate-in fade-in duration-300">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between pb-4 border-b border-zinc-100 dark:border-zinc-800 gap-2">
                  <div>
                    <h3 className="text-sm font-black uppercase tracking-wider text-zinc-855 dark:text-white">Desempenho por Assunto</h3>
                    ...
                    <div className="divide-y divide-zinc-100 dark:divide-zinc-850">
                      ...
                      <div className="flex items-center justify-between p-3.5 pl-6">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[8px] font-black text-amber-500 bg-amber-50 dark:bg-amber-955/40 border border-amber-200/30 dark:border-amber-900/20 px-1.5 py-0.5 rounded uppercase">Prioridade Média</span>
                        </div>
                        <span className="text-xs font-black text-amber-555 dark:text-amber-400">67% de acerto</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {showAuth && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-lg animate-in fade-in duration-300">
          ...
          >
            ...
            {/* Selo Anti-Phishing de Domínio Oficial */}
            <div className="bg-emerald-50/80 dark:bg-emerald-950/30 border border-emerald-200/60 dark:border-emerald-800/40 p-2.5 rounded-2xl flex items-center gap-2 mb-4 text-[10px] text-emerald-800 dark:text-emerald-300 font-bold shadow-xs">
              <ShieldCheck size={14} className="text-emerald-500 shrink-0" />
              <span>Domínio Oficial Verificado: <strong className="font-mono text-[9px] text-emerald-600 dark:text-emerald-400">{window.location.host}</strong></span>
            </div>
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 text-zinc-800 dark:text-white py-3 rounded-2xl font-bold text-xs hover:bg-zinc-50 dark:hover:bg-zinc-750 transition-all flex items-center justify-center gap-2 shadow-xs mb-4"
            >
              ...
              {isRegistering ? 'Cadastrar com o Google' : 'Continuar com o Google'}
            </button>
            ...
            <div className="relative my-4">
              ...
              <div className="relative flex justify-center text-[9px] uppercase"><span className="bg-white dark:bg-zinc-900 px-2 text-zinc-400 font-black">ou e-mail</span></div>
            </div>

            {/* Formulário de Acesso */}
            <form onSubmit={handleSubmit} className="space-y-4">
              <input type="text" name="website_url_hp" value={honeypot} onChange={(e) => setHoneypot(e.target.value)} className="hidden" tabIndex={-1} autoComplete="off" />
              <div className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3.5 bg-zinc-50 dark:bg-zinc-955 border border-zinc-200/75 dark:border-zinc-800/85 rounded-2xl outline-none focus:ring-2 focus:ring-blue-600 dark:text-white font-medium text-xs transition-all"
                    required
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  {isRegistering && password.length > 0 && (
                    <div className="flex items-center gap-1.5 mt-1">
                      <div className={`h-1 flex-1 rounded-full ${password.length >= 8 ? 'bg-emerald-500' : password.length >= 6 ? 'bg-amber-500' : 'bg-rose-500'}`} />
                      <span className="text-[8px] font-black uppercase text-zinc-400">
                        {password.length >= 8 ? 'Forte' : password.length >= 6 ? 'Média' : 'Curta'}
                      </span>
                    </div>
                  )}
                </div>
              ...
              {successMsg && (
                <p className="text-[9px] font-black text-emerald-555 uppercase bg-emerald-50 dark:bg-emerald-900/10 p-2.5 rounded-xl text-center leading-normal border border-emerald-255/10">
                  {successMsg}
                </p>
              )}
            ...
            <p className="text-center text-xs font-medium text-zinc-550 mt-6">
